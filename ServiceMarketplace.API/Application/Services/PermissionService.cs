...
{
    // L1 (IMemoryCache): avoids ~270 ms Redis round-trip on every authenticated request.
    // L2 (Redis): authoritative copy; L1 serves as a short-lived front.
    ...
    public async Task<HashSet<string>> GetEffectivePermissionsAsync(Guid userId)
    {
        ...
        {
            // Admin always has every defined permission.
    ...
    // Read order: L1 memory → L2 Redis → DB.
    ...
    private async Task<HashSet<string>> GetEffectivePermissionsInternalAsync(UserRole role, Guid userId)
    {
        ...
        // DB path: role baseline from L2/DB, then apply per-user overrides.
        ...
        // Apply explicit user-level overrides (grant/revoke on top of role baseline).
